...
TYPES: BEGIN OF ty_invoice_not_approval,
         ...
         txt20        TYPE char20,           " G/L Account Short Text
...
PARAMETERS: p_not_app RADIOBUTTON GROUP rg1 DEFAULT 'X' USER-COMMAND radio,
...
" Global Variables
"----------------------------------------------------------------------
DATA: go_alv_handler TYPE REF TO lcl_main,
...
START-OF-SELECTION.
  ...
  IF p_not_app = 'X'.
    go_alv_handler->display_not_approval( ).
  ELSE.
    go_alv_handler->display_in_approval( ).
  ENDIF.
